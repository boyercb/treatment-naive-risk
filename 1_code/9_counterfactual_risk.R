...
risk_factor_means <- 
  risk_df %>%
  summarise(across(all_of(baseline_vars_long), mean))
...
p3 <- 
  ggplot(risk_df, aes(x = value, color = name)) +
  geom_line(aes(y = 1 - ..y..), stat='ecdf') +
  theme_classic() +
  # annotate("text", x = 0.085, y = 0.54, label = "0.53", ) +
  scale_x_continuous(breaks = seq(0, 0.6, by = 0.05), minor_breaks = T) +
...
p4 <-
  ggplot(nhanes_data, aes(x = value, color = name)) +
  geom_line(aes(y = 1 - ..y..), stat='ecdf') +
  theme_classic() +
  # annotate("text", x = 0.085, y = 0.54, label = "0.53", ) +
  scale_x_continuous(breaks = seq(0, 0.6, by = 0.05), minor_breaks = T) +
